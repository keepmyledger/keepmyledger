...
describe('SqliteTransactionRepo', () => {
  ...
  it('bulkCreate inserts new rows and skips duplicates by external_hash', async () => {
    ...
    const r2 = await h.owner.transactions.bulkCreate([
      makeTx(ownerAcctId, ownerStmtId, '2026-01-05', 'COFFEE', -4.5), // dup
      makeTx(ownerAcctId, ownerStmtId, '2026-01-07', 'BOOK',   -12),  // new
    ]);
    expect(r2).toEqual({ inserted: 1, skipped: 1 });

    expect((await h.owner.transactions.findAll()).length).toBe(3);
  });
  ...
  it('update recomputes external_hash when dedup fields change', async () => {
    ...
    // Re-importing the original line should now NOT dedup (because hash changed)
    const r = await h.owner.transactions.bulkCreate([makeTx(ownerAcctId, ownerStmtId, '2026-01-05', 'COFFEE', -4.5)]);
